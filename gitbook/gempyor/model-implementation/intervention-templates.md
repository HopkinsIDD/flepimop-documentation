...
{% hint style="info" %}
Note that if you want a parameter to vary continuously over time (for example, a daily transmission rate that is influenced by temperature and humidity, or a testing rate that fluctuates rapidly due to test availability), then it is easier to do this by using a "timeseries" parameter value than by combining many separate modifiers. Timeseries parameter values are described for both the [seir::parameters](compartmental-model-structure.md#specifying-compartmental-model-transitions-seir-transitions) and [`outcomes`](outcomes-for-compartments.md) sections.&#x20;
{% endhint %}
