...
\begin{algorithmic}[1]
\State \textbf{Inputs:} initial parameter distribution $\mathcal{I}(\cdot)$, proposal distribution $g(\Theta)$, prior distribution $p(\Theta)$, \texttt{gempyor} epidemic model $Z(\Theta)$, likelihood function and ground-truth data $\mathcal{L}(D|Z(\Theta))$ 
...
%s\item[]
  \For{$k=1  \dots K$} \Comment{$K$: length of the MCMC chain}
    ...
    %\State Calculate the overall likelihood with the proposed parameters, $\mathcal{L}(D|Z(\Theta^*))$
      ...
      \If{$\alpha^G > u \sim \mathcal{U}[0,1]$} \Comment{\emph{Accept} proposal to the global and chimeric parameter chains}
        \State $\Theta^G_{m,k} \gets \Theta^*$
        \State $\Theta_{m,k}^C \gets \Theta^*$
      \Else  \Comment{\emph{Reject} proposal for the global chain}
